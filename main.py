...
#Print de jugadores
  ...
  print(RED+"Instrucciones","\n Cada punto correcto vale 10 puntos\n Cada punto incorrecta son -5 puntos")
  print("Debes esperar a que el contador pare para poder escribir tu respuesta")
  print("Solo se escogera a uno de los jugadores de forma aleatoria, suerte!"+RESET)
  ...
  print(YELLOW+"\nSe escogio aleatoriamente los puntos y comenzaras con:\n",puntaje, "Puntos", BLUE+jugadores[i]+RESET)
  ...
  if respuesta == "a":
    puntaje += 10
    print(YELLOW+"Muy bien!" ,jugadores[i], "ganaste 10 puntos sigue jugando")
    print("Puntaje total:"+RESET, puntaje)
    
   #Si no escogio la respuesta correcta se lo mostramos
  else:
    puntaje -= 5
    print(YELLOW+"Incorrecto" ,jugadores[i], " Perdiste 5 puntos!")
    print(RESET+"Puntaje total:", puntaje)
  ...
  if respuesta == "a":
    puntaje += 10
    print ("Muy bien",jugadores[i],"ganaste 10 puntos")
    print(YELLOW+"\nPuntuacion total:", puntaje, "gracias por jugar"+RESET)
  
  else:
     puntaje -= 5
     print ("Incorrecto!", jugadores[i], "Perdiste 5 puntos, La torre Eiffel se encuentra en francia")
     print(YELLOW+"\nPuntuacion total", puntaje, "gracias por jugar"+RESET)
  
  #Creamos un bonus de puntos que puede ganar escribiendo ruleta
  bonus = input(YELLOW+"\nEscribe ruleta para obtener un bonus de puntos aleatorio "+RESET)

  #Si no escribe correctamente se le mostrara al usuario
  while bonus not in ("ruleta"):
    bonus = input("No escribiste correctamente, intenta nuevamente")

  #Si escribio ruleta se le asigna puntos random
  if bonus == "ruleta":
    bono = random.randint(0, 20)
    print(BLUE+"Felicidades ",jugadores[i], "ganaste" ,bono, "puntos de bono"+RESET)
...
#Si escribe si, sigue jugando
    if game == "si":
      jugar= True
        ...
        #Si escribe no, Se termina el juego
        jugar= False
